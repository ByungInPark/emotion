...
export default function FindIdPage() {
    ...
    const handleSendCode = () => {
      if (method === 'phone' && phone) {
        const generated = Math.floor(100000 + Math.random() * 900000).toString(); // 예시 코드
        setSentCode(generated);
        alert(`인증번호 발송됨: ${generated}`);
      } else if (method === 'email' && email) {
        const generated = Math.floor(100000 + Math.random() * 900000).toString();
        setSentCode(generated);
        alert(`이메일로 인증번호 발송됨: ${generated}`);
      }
    };
    ...
    return (
      <div className="max-w-2xl mx-auto bg-white p-8 rounded shadow mt-10">
          ...
          <>
            <div className="flex gap-2 mb-4">
              ...
              <button
                ...
                disabled={!phone}
                className={`px-4 py-2 rounded text-white ${phone ? 'bg-blue-500' : 'bg-gray-400'}`}
              >
                인증번호 받기
              </button>
            </div>
          ...
          <>
            <div className="flex gap-2 mb-4">
              ...
              <button
                onClick={handleSendCode}
                disabled={!email}
                className={`px-4 py-2 rounded text-white ${email ? 'bg-blue-500' : 'bg-gray-400'}`}
              >
                인증번호 받기
              </button>
            </div>
        ...
        <button
          className={`w-full py-2 rounded text-white ${code.length === 6 ? 'bg-purple-500' : 'bg-gray-400'}`}
